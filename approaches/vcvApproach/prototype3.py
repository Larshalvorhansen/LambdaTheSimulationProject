...
class Port:
    def __init__(self, block, name, kind, x, y, external=False):
        self.block = block
        self.name = name
        ...
        self.external = external  # True if it can be driven by slider
...
class Block:
    ...
    def add_input(self, name, yoff, external=False):
        p = Port(self, name, "in", 15, yoff, external)
...
class App:
    def __init__(self, root):
        ...
        self.canvas = tk.Canvas(self.main_frame, width=700, height=500, bg="#111")
        self.canvas.pack(side="left", fill="both", expand=True)
        self.sidebar = tk.Frame(self.main_frame, width=200, bg="#222")
        ...
        self.p_subsidies = self.company.add_input("Subsidies", 60, external=True)
        self.p_revenues = self.company.add_input("Revenues", 110, external=True)
        self.company.add_input("Worker output", 160)
        self.company.add_output("Gov taxes", 80)
        self.company.add_output("Salary", 140)

        self.worker = Worker(self.canvas, 400, 100, 250, 200, "Worker Y")
        self.worker.add_input("Salary", 60)
        self.p_satisfaction = self.worker.add_input("Satisfaction", 110, external=True)
        self.p_goods = self.worker.add_input("Goods", 160, external=True)
        ...
        # Sliders for external inputs
        self.sliders = {}
        self._make_slider("Company Subsidies", self.p_subsidies, 0, 10000, 1000)
        self._make_slider("Company Revenues", self.p_revenues, 0, 10000, 5000)
        self._make_slider(
            "Worker Satisfaction (0-1)", self.p_satisfaction, 0, 1, 0.7, step=0.01
        )
        self._make_slider("Worker Goods", self.p_goods, 0, 10000, 3000)
    ...
    # Sidebar slider
    # --------------------
    def _make_slider(self, label, port, mn, mx, init, step=1):
        frame = tk.Frame(self.sidebar, bg="#222")
        frame.pack(pady=8, padx=8, anchor="w")
        tk.Label(frame, text=label, fg="white", bg="#222").pack(anchor="w")
        s = tk.Scale(
            frame,
            from_=mn,
            to=mx,
    ...
    def update_loop(self):
        # Reset input values (to slider values if external)
        for b in self.blocks:
            for p in b.inputs:
                p.value = self.sliders.get(p, tk.Scale()).get() if p.external else 0.0

        # Aggregate from cables
        ...
        for b in self.blocks:
            ...
            for outv, port in zip(results.values(), b.outputs):
                port.value = outv
